  ...
  )
{
  ...
  TdxWorkArea = (TDX_WORK_AREA *)FixedPcdGet32 (PcdTdWorkAreaBase);
  ...
  )
{
  ...
  //
  mTdxSupported = CheckTdxSupported(&TdInitVp, &TdInfo);
  SetTdGuest(mTdxSupported);

  if(mTdxSupported){

    TdxInitialize(&SecCoreData, BootFv, TopOfCurrentStack);

    TdxStartup(&SecCoreData, TdInitVp, TdInfo);
